/**
 * graph-memory — LLM prompts for extraction and finalization
 *
 * SRP: prompt text only. No logic, no state. Kept separate so the Extractor
 * class stays focused on orchestration and parsing.
 */

// ─── 提取 System Prompt ─────────────────────────────────────────

export const EXTRACT_SYS = `你是 graph-memory 知识图谱提取引擎，从 AI Agent 对话中提取可复用的结构化知识三元组（节点 + 关系）。
...
export const EXTRACT_USER = (msgs: string, existing: string) =>
`<Existing Nodes>
${existing || "（无）"}
...
export const FINALIZE_SYS = `你是图谱节点整理引擎，对本次对话产生的节点做 session 结束前的最终审查。
...
export const FINALIZE_USER = (nodes: any[], summary: string) =>
`<Session Nodes>
